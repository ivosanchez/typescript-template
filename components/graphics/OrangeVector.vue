<template>
  <svg ref="svg" class="vector" :class="classes" :viewBox="setViewbox()" @mousemove="setOffset">
<g id='g'><use href='#s' transform='scale(0.12) rotate(60)'/><use href='#s' transform='scale(0.2) rotate(10)'/><use href='#s' transform='scale(0.25) rotate(40)'/><use href='#s' transform='scale(0.3) rotate(-20)'/><use href='#s' transform='scale(0.4) rotate(-30)'/><use href='#s' transform='scale(0.5) rotate(20)'/><use href='#s' transform='scale(0.6) rotate(60)'/><use href='#s' transform='scale(0.7) rotate(10)'/><use href='#s' transform='scale(0.835) rotate(-40)'/><use href='#s' transform='scale(0.9) rotate(40)'/><use href='#s' transform='scale(1.05) rotate(25)'/><use href='#s' transform='scale(1.2) rotate(8)'/><use href='#s' transform='scale(1.333) rotate(-60)'/><use href='#s' transform='scale(1.45) rotate(-30)'/><use href='#s' transform='scale(1.6) rotate(10)'/></g></defs><g transform='translate(220 0)'><g transform='translate(0 450)'><circle fill='url(#a)' r='3000'/><g opacity='0.5'><circle fill='url(#a)' r='2000'/><circle fill='url(#a)' r='1800'/><circle fill='url(#a)' r='1700'/><circle fill='url(#a)' r='1651'/><circle fill='url(#a)' r='1450'/><circle fill='url(#a)' r='1250'/><circle fill='url(#a)' r='1175'/><circle fill='url(#a)' r='900'/><circle fill='url(#a)' r='750'/><circle fill='url(#a)' r='500'/><circle fill='url(#a)' r='380'/><circle fill='url(#a)' r='250'/></g><g transform='rotate(-216 0 0)'><use href='#g' transform='rotate(10)'/><use href='#g' transform='rotate(120)'/><use href='#g' transform='rotate(240)'/></g><circle fill='url(#a)' r='3000'/></g></g>
    ...
    <transition name="vector-images">
      <g v-if="!hideImages && !mobileImages">
        <image
          ...
          :transform="`translate(${points[image.key]})`"
...
export default {
  ...
  mounted () {
    if (!this.animated) { this.setPoints() } else if (this.$store.state.vector.initialized) { this.resumeAnimation() } else { setTimeout(this.initAnimation, 450) }
  },
  ...
  methods: {
    ...
    setViewbox () {
      if (!this.viewbox) {
        ...
        const [offX, offY, sizeX, sizeY] = this.references.viewbox
        return [
          vbox.offsetX || offX,
          vbox.offsetY || offY,
          vbox.sizeX || sizeX,
    ...
    initAnimation () {
      ...
      while (keys.length) {
        const ref = this.references.points[keys[0]]
        const refs = {}
    ...
    setAnimation (key, ref) {
      const { duration, dest } = this.generateRandom(ref)
    ...
    setOffset (e) {
      if (this.animated) {
        const { cX, cY } = this.rebase
        this.offset.x = (e.pageX - cX) / cX * 40 | 0
        this.offset.y = (e.pageY - cY) / cY * 40 | 0
      }
    },
    mapToScreen ([refX, refY]) {
      const ref = this.$refs.svg.createSVGPoint()
      ref.x = refX
      ref.y = refY
      const { x, y } = ref.matrixTransform(this.$refs.svg.getScreenCTM())
      return [x | 0, y | 0]
    }
  }
}
